...
init()
{
  std::cerr << GetTheApplication()->GetRank() << ": libgxy_module_globals.so\n";
  PrintTestObjectMsg::Register();
  ShowKeyedObjectsMsg::Register();
  TestObject::Register();
  MultiServer::Register();
}
...
server(MultiServerHandler *handler)
{
  ...
  while (! client_done)
  {
    ...
    if (cmd == "add")
    {
      ...
      to->set_string(name);
      to->Commit();
      MultiServer::Get()->SetGlobal(name, to);
    }
    else if (cmd == "show")
    {
      string name;
      ss >> name;
      TestObjectP to = TestObject::Cast(MultiServer::Get()->GetGlobal(name));
      if (to)
        ShowKeyedObject(to);
      else
      ...
      MultiServer::Get()->getTheDynamicLibraryManager()->Show();
    else if (cmd == "break")
      brk();
    else if (cmd == "drop")
    {
      ...
      {
        cerr << "dropping " << name << "\n";
        MultiServer::Get()->DropGlobal(name);
      }
      else
        cerr << "no such object: " << name << "\n";
    }
