...
class MultiServerHandler
{
  ...
  static MultiServerHandlerP NewP(int cfg, int dfd) { return MultiServerHandlerP(new MultiServerHandler(cfg, dfd)); }
  static MultiServerHandlerP NewP(const char *h, int port) { return MultiServerHandlerP(new MultiServer(host, port)); }

private:
  MultiServerHandler(int cfd, int dfd) : data_fd(dfd) , control_fd(cfd) {}
  MultiServerHandler(const char* host, int port);
