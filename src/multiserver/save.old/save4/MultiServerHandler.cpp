...
MultiServerHandler::MultiServerHandler(int cfd, int dfd) : MultiServerHandler::MultiServerHandler() 
{
  data_fd = dfd;
  control_fd = cfd;
}
...
MultiServerHandler::~MultiServerHandler()
{
  if (dlp) dlp->Deref();  // Remove this handler's reference on the dynamic lib
...
MultiServerHandler::RunServer()
{
  ...
  dlp = GetTheApplication()->GetTheDynamicLibraryManager()->Load(libname);
  free(libname);
