...
MultiServer::start(void *d)
{
  ...
  {
    DynamicLibraryP dlp = GetTheApplication()->GetTheDynamicLibraryManager()->Load(buf);
    free(buf);

    typedef bool (*server_function_t)(MultiServerHandler *);
    server_function_t server_function = (server_function_t)dlp->GetSym("server");
    ...
    {
      std::string ok("ok");
      msh->CSend(ok.c_str(), ok.length()+1);
      if (! server_function(msh))
