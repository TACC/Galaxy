...
main(int argc, char **argv)
{
	...
	string dir =(vname.find_last_of("/") == vname.npos) ? "./" : vname.substr(0, vname.find_last_of("/") + 1);
	...
	for (int i = mpi_rank; i < np; i += mpi_size)
	{
		grid* p = partitions + i;
    ...
    cerr << "stepsize " << p->stepsize.x << " " << p->stepsize.y << " " << p->stepsize.z << "\n";
		...
		for (int i = 0; i < contours.size(); i++)
			cf->SetValue(i, contours[i]);

		cf->Update();
		cf->GetOutput()->RemoveGhostCells();

		vtkXMLPolyDataWriter *pdw = vtkXMLPolyDataWriter::New();

		pdw->SetFileName((ss.str() + ".vtp").c_str());
		pdw->SetInputConnection(cf->GetOutputPort());
		pdw->Write();
