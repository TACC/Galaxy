...
namespace ospray {
  ...
  /*! \defgroup geometry_trianglemesh Triangle Meshes ("trianglemesh")
    ...
    \ref ospray::DataDrivenTriangleMesh class.
  */
   ...
   */
  struct OSPRAY_SDK_INTERFACE DataDrivenTriangleMesh : public Geometry
  {
    DataDrivenTriangleMesh();
    virtual ~DataDrivenTriangleMesh() override = default;
    virtual std::string toString() const override;
    virtual void finalize(Model *model) override;

    int    *index;  //!< mesh's triangle index array
    float  *vertex; //!< mesh's vertex array
    float  *data; //!< mesh's per-vertex data array
    ...
    Ref<Data> dataData; /*!< vertex texcoord array (vec2f) */
