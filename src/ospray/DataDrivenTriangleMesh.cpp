...
// ospray
#include "DataDrivenTriangleMesh.h"
#include "transferFunction/TransferFunction.h"
#include "common/Model.h"
#include "../include/ospray/ospray.h"
// ispc exports
#include "DataDrivenTriangleMesh_ispc.h"
#include <cmath>
...
namespace ospray {
  ...
  DataDrivenTriangleMesh::DataDrivenTriangleMesh()
  {
    this->ispcEquivalent = ispc::DataDrivenTriangleMesh_create(this);
  }

  std::string DataDrivenTriangleMesh::toString() const
  {
    return "ospray::DataDrivenTriangleMesh";
  }

  void DataDrivenTriangleMesh::finalize(Model *model)
  {
    ...
    this->data  = dataData ? (float*)dataData->data : nullptr;
    ...
    if (this->data)
    {
      transferFunction = (TransferFunction *)getParamData("transferFunction", nullptr);
      if (! transferFunction) { std::cerr << "per-vertex data on trriangle mesh but no TF\n"; this->data = NULL; }
    }


    ispc::DataDrivenTriangleMesh_set(getIE(),model->getIE(),
                           eMeshGeom,
                           eMeshID,
                           numTris,
                           numCompsInTri,
  ...
  OSP_REGISTER_GEOMETRY(DataDrivenTriangleMesh,ddtriangles);
  OSP_REGISTER_GEOMETRY(DataDrivenTriangleMesh,ddtrianglemesh);
